...
def perform_ocr(image_path):
    ...
    try:
        ...
        # STAGE 1: Plate Detection (using CRAFT via EasyOCR detection)
        # We get bounding boxes of text regions
        detection_results = reader.detect(img)

        # EasyOCR's detect returns (horizontal_list, free_list)
        # horizontal_list format: [ [x_min, x_max, y_min, y_max], ... ]
        # free_list format: [ [[x1,y1],[x2,y2],[x3,y3],[x4,y4]], ... ]

        boxes = detection_results[0][0] # Focus on the first detected horizontal box if available
        if not boxes:
            # Fallback to recognition on the whole image if no box detected
        ...
        # Use the box with the largest area (most likely the plate)
        x_min, x_max, y_min, y_max = boxes

        # Add some padding for better recognition context
        padding = 10
        h, w, _ = img.shape
        ...
        # STAGE 2: Targeted Pre-processing of the Crop
        # 1. Upscale the crop for character clarity
        ...
        # STAGE 3: CNN Recognition on high-quality crop
